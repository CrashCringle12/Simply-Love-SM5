			...
			CharacterChangedMessageCommand=function(self,param)
				if param.Player == PLAYER_1 then
					if param.Character then
						local name = param.Character:GetDisplayName()
						local bSpecial=false
						self:settext(name);
						for _,v in pairs(specialCharacters) do
							if v == name then bSpecial = true end
						end
						self:diffuse(bSpecial and color("#FFD600") or PlayerColor(PLAYER_1))
						-- todo: change diffuse if special character
						-- (Naoki, NMR, 2MB, U1)
						-- self:diffuse(color("#FFD600"))
					end;
				end
			end;
		};
