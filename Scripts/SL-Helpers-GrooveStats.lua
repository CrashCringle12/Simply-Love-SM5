...
-- -----------------------------------------------------------------------
local IsValidGrooveStatsApiKey = function(apiKey)
	return type(apiKey) == "string" and #apiKey == 64
end

local DEBUG_GROOVESTATS_CARD_PARSING = true

local function DebugGrooveStatsCardParsing(message)
	if not DEBUG_GROOVESTATS_CARD_PARSING then return end
	if type(SCREENMAN) == "table" and type(SCREENMAN.SystemMessage) == "function" then
		SCREENMAN:SystemMessage("[GrooveStatsCard] " .. tostring(message))
	else
		SM("[GrooveStatsCard] " .. tostring(message))
	end
end
...
local IsNFCCardDataIOSupported = function()
	if NFCMAN == nil then return false end
	if type(NFCMAN.SupportsCardDataIO) ~= "function" then return false end

	if type(NFCMAN.IsEnabled) == "function" and not NFCMAN:IsEnabled() then
		return false
	end

	return NFCMAN:SupportsCardDataIO() == true
end

local ReadGrooveStatsIniByProfileId = function(profile_id)
	if type(profile_id) ~= "string" or profile_id == "" then return nil end

	local dir = PROFILEMAN:LocalProfileIDToDir(profile_id)
	...
	local contents = IniFile.ReadFile(path)
	local gs = (type(contents) == "table" and type(contents.GrooveStats) == "table") and contents.GrooveStats or {}
	local apiKey = type(gs.ApiKey) == "string" and gs.ApiKey or ""
	if not IsValidGrooveStatsApiKey(apiKey) then
		apiKey = ""
	end

	return {
		ApiKey = apiKey,
		Username = type(gs.Username) == "string" and gs.Username or "",
		IsPadPlayer = (gs.IsPadPlayer == 1 or gs.IsPadPlayer == "1") and 1 or 0,
		path = path,
	}
end

local WriteGrooveStatsIniByProfileId = function(profile_id, gsData)
	if type(profile_id) ~= "string" or profile_id == "" then return false end
	if type(gsData) ~= "table" then return false end
	...
	IniFile.WriteFile(path, {
		["GrooveStats"] = {
			["ApiKey"] = IsValidGrooveStatsApiKey(gsData.ApiKey) and gsData.ApiKey or "",
			["Username"] = type(gsData.Username) == "string" and gsData.Username or "",
			["IsPadPlayer"] = (gsData.IsPadPlayer == 1 or gsData.IsPadPlayer == "1" or gsData.IsPadPlayer == true) and "1" or "0",
		},
	})
...
local ParseGrooveStatsCardData = function(rawData)
	DebugGrooveStatsCardParsing("rawData type=" .. type(rawData))

	if type(rawData) == "table" then
		DebugGrooveStatsCardParsing("rawData keys=" .. TableKeysToString(rawData))
		if type(rawData.GrooveStats) == "table" or type(rawData.ApiKey) == "string" or type(rawData.apiKey) == "string" then
			local gs = type(rawData.GrooveStats) == "table" and rawData.GrooveStats or rawData
			local apiKey = type(gs.ApiKey) == "string" and gs.ApiKey or (type(gs.apiKey) == "string" and gs.apiKey or "")
			if not IsValidGrooveStatsApiKey(apiKey) then
				apiKey = ""
			end

			DebugGrooveStatsCardParsing("parsed direct table payload apiKeyLen=" .. tostring(#apiKey))
			return {
				ApiKey = apiKey,
				Username = type(gs.Username) == "string" and gs.Username or (type(gs.username) == "string" and gs.username or ""),
				IsPadPlayer = (gs.IsPadPlayer == 1 or gs.IsPadPlayer == "1" or gs.isPadPlayer == 1 or gs.isPadPlayer == "1"),
			}
		end

		if type(rawData.data) == "string" then
			rawData = rawData.data
		elseif type(rawData.payload) == "string" then
			rawData = rawData.payload
		elseif type(rawData.cardData) == "string" then
			rawData = rawData.cardData
		else
			DebugGrooveStatsCardParsing("table payload had no string data field")
			return nil
		end
	end

	if type(rawData) ~= "string" or rawData == "" then
		DebugGrooveStatsCardParsing("raw payload was empty or not string after normalization")
		return nil
	end

	DebugGrooveStatsCardParsing("raw payload len=" .. tostring(#rawData))

	local decoded = JsonDecode(rawData)
	if type(decoded) ~= "table" then
		DebugGrooveStatsCardParsing("JsonDecode failed or returned non-table")
		return nil
	end

	DebugGrooveStatsCardParsing("decoded keys=" .. TableKeysToString(decoded))

	local gs = type(decoded.GrooveStats) == "table" and decoded.GrooveStats or decoded
	if type(gs) ~= "table" then
		DebugGrooveStatsCardParsing("decoded payload did not contain GrooveStats table")
		return nil
	end

	DebugGrooveStatsCardParsing("GrooveStats keys=" .. TableKeysToString(gs))

	local apiKey = type(gs.ApiKey) == "string" and gs.ApiKey or (type(gs.apiKey) == "string" and gs.apiKey or "")
	if not IsValidGrooveStatsApiKey(apiKey) then
		DebugGrooveStatsCardParsing("ApiKey missing or invalid, len=" .. tostring(type(apiKey) == "string" and #apiKey or -1))
		apiKey = ""
	else
		DebugGrooveStatsCardParsing("ApiKey parsed successfully")
	end
...
ReadGrooveStatsCardData = function()
	if not IsNFCCardDataIOSupported() then return nil end

	local ok, rawData = pcall(function() return NFCMAN:ReadCardData() end)
	if not ok then return nil end

	if type(rawData) == "table" then
		rawData = type(rawData.data) == "string" and rawData.data or ""
	end

	return ParseGrooveStatsCardData(rawData)
end

ReadGrooveStatsCardApiKey = function()
	local cardData = ReadGrooveStatsCardData()
	if type(cardData) ~= "table" then return "" end
	return IsValidGrooveStatsApiKey(cardData.ApiKey) and cardData.ApiKey or ""
end

GetGrooveStatsProfileApiKey = function(profile_id)
	local profileData = ReadGrooveStatsIniByProfileId(profile_id)
	if type(profileData) ~= "table" then return "" end
	return IsValidGrooveStatsApiKey(profileData.ApiKey) and profileData.ApiKey or ""
end

WriteGrooveStatsCardData = function(profile_id)
	if not IsNFCCardDataIOSupported() then return false end

	local profileData = ReadGrooveStatsIniByProfileId(profile_id)
	if type(profileData) ~= "table" then return false end
	if not IsValidGrooveStatsApiKey(profileData.ApiKey) then return false end

	local payload = JsonEncode({
		GrooveStats = {
			ApiKey = profileData.ApiKey,
			Username = profileData.Username or "",
			IsPadPlayer = (profileData.IsPadPlayer == 1 or profileData.IsPadPlayer == true) and 1 or 0,
		}
	})

	local ok, result = pcall(function() return NFCMAN:WriteCardData(payload) end)
	if not ok then return false end

	return result ~= false
end

UpdateGrooveStatsIniFromCardData = function(profile_id)
	local profileData = ReadGrooveStatsIniByProfileId(profile_id)
	if type(profileData) ~= "table" then return false end

	-- Only import from card when the linked local profile currently has no valid key.
	if IsValidGrooveStatsApiKey(profileData.ApiKey) then return false end

	local cardData = ReadGrooveStatsCardData()
	if type(cardData) ~= "table" then return false end
	if not IsValidGrooveStatsApiKey(cardData.ApiKey) then return false end

	return WriteGrooveStatsIniByProfileId(profile_id, {
		ApiKey = cardData.ApiKey,
		Username = cardData.Username or "",
		IsPadPlayer = cardData.IsPadPlayer and 1 or 0,
	})
end
